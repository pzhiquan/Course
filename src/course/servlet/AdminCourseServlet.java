...
public class AdminCourseServlet extends HttpServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Course course = CommonUtils.toBean(map, Course.class);//把Map中大部分数据封装到course对象中
		 ...
		 */
		// 获取文件名
		FileItem fileItem = fileItemList.get(10);//获取图片
		...
		// 把图片的路径设置给book对象
		course.setCourseImages("/bs-course/images/" + filename);
		...
		// 保存成功信息转发到msg.jsp
		request.setAttribute("msg", "修改课程成功！");
		request.getRequestDispatcher("/CourseServlet?method=findByAllM&all=1").forward(request, response);
	}
