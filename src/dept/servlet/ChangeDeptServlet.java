...
public class ChangeDeptServlet extends HttpServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Dept dept = CommonUtils.toBean(map,Dept.class);//把Map中大部分数据封装到admin对象中
		//dept.setDeptID(CommonUtils.uuid());
		DeptService deptService=new DeptService();
		deptService.change(dept);
		//DeptService.add(dept);
		request.setAttribute("msg", "修改学院成功！");
		request.getRequestDispatcher("/DeptServlet?method=findAllB").forward(request, response);
