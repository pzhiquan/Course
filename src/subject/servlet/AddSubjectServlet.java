...
public class AddSubjectServlet extends HttpServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Subject subject = CommonUtils.toBean(map,Subject.class);//把Map中大部分数据封装到admin对象中
		subject.setSubjectID(CommonUtils.uuid());
		SubjectService subjectService=new SubjectService();
		//coursetypeService.change(courseType);
		subjectService.add(subject);
		request.setAttribute("msg", "修改课程类别成功！");
		request.getRequestDispatcher("/SubjectServlet?method=findAllB").forward(request, response);
